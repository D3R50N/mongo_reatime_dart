...
class RealtimeQueryManager {
  ...
  void applyInitial(String queryId, List<JsonMap> documents) {
    ...
    for (final document in documents) {
      _cacheManager.upsert(liveQuery.definition.collection, document);
    }
    ...
    liveQuery.replaceDocuments(next, forceEmit: true);
  }

  void applyInsert(String collection, JsonMap document) {
    _cacheManager.upsert(collection, document);

    for (final liveQuery in _queries.values) {
      if (liveQuery.definition.collection != collection) {
        continue;
      }
  ...
  void applyUpdate({
    ...
    _cacheManager.upsert(collection, current);
  ...
  void applyDelete({required String collection, required String id}) {
    _cacheManager.delete(collection, id);
    ...
    for (final liveQuery in _queries.values) {
      ...
      liveQuery.replaceDocuments(_backfillFromCache(liveQuery, next));
    }
  }
  ...
  void _applyDocumentChange(
    ...
    final backfilled = _backfillFromCache(liveQuery, finalized);
    liveQuery.replaceDocuments(backfilled);
  }
  ...
  List<JsonMap> _backfillFromCache(
    ...
    final candidates = <JsonMap>[
      ...
      ..._cacheManager
          ...
          .where((document) => !seedIds.contains(document['_id']?.toString())),
    ];
